...
namespace UnrealRuby {
	...
	{
		UR_CORE_ERROR("GLFW Error ({0}):{1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		UR_CORE_INFO("Creating Window {0} ({1},{2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			//TODO: glfwterminate on system shutdown
			int success = glfwInit();
			UR_CORE_ASSERT(success, "Could not Initialize GLFW!");
